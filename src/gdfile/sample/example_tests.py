...
def test_{{main_class}}_format(
    ...
    """Формат {{MainClass}}."""
    url = reverse('{{app-name}}:{{main_1-class}}-detail', [{{main_class}}.pk])
...
def test_{{main_class}}_post(
    ...
    """Создание {{MainClass}}."""
    url = reverse('{{app-name}}:{{main_1-class}}-list')
...
def test_{{main_class}}_change(
    ...
    """Изменение {{MainClass}}."""
    url = reverse('api:{{app-name}}:{{main_1-class}}-detail', [{{main_class}}.pk])
